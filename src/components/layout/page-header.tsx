...
export function PageHeader({
  ...
  backHref?: string
  /** Intercept the back control (e.g. to guard unsaved changes). Takes
   *  precedence over backHref. */
  onBack?: () => void
  className?: string
  ...
  const backClass =
    'grid size-8 shrink-0 place-items-center rounded-[9px] border border-border text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground'
  return (
      ...
      <div className="min-w-0">
        ...
        <div className="flex items-center gap-3">
          {onBack ? (
            ...
            <a href={backHref} className={backClass} aria-label="Back">
              {backIcon}
            </a>
          ) : null}
