...
interface BrandAssetSelectorProps {
  ...
  maxSelection?: number
  apiPath?: string // Defaults to '/api/creative-hub/assets'
}
...
export function BrandAssetSelector({
  ...
  async function loadAssets() {
    try {
      ...
      const res = await fetch(`${apiPath}${catParam}`)
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>
            Select Brand Assets
            <span className="text-sm font-normal font-mono tabular-nums text-muted-foreground ml-2">
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {loading ? (
              ...
              {filteredAssets.map((asset) => {
                ...
                return (
                  <button
                    ...
                    disabled={isDisabled}
                    className={cn(
                      'relative rounded-[10px] border overflow-hidden transition-all text-left',
                      isSelected
                        ? 'border-gold-600/50 ring-2 ring-gold-600/30 dark:border-gold-400/40 dark:ring-gold-400/25'
                        : isDisabled
            ...
            <div className="text-center py-12">
              ...
              <a
                href="/studio/assets"
                className="text-sm text-primary hover:opacity-80 underline mt-1 inline-block"
              >
                Upload assets
              </a>
            </div>
        ...
        <div className="flex justify-between items-center pt-3 border-t border-border">
          ...
          <div className="flex gap-1.5">
            {localSelected.map((id) => {
              ...
              return (
                <div key={id} className="relative w-8 h-8 rounded-[8px] overflow-hidden border border-border">
                  <img src={asset.imageUrl} alt={asset.name} className="w-full h-full object-cover" />
                  <button
                    onClick={() => toggleAsset(id)}
                    className="absolute inset-0 bg-ink-950/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity"
                  >
