...
export function ActivityFeed({ activities }: ActivityFeedProps) {
  ...
  const getOutcomeBadge = (outcome: string) => {
    switch (outcome) {
      ...
      case 'follow_up':
        return <Badge variant="outline" className="rounded-sm text-[10px] px-1.5 py-0 border-warm-300 dark:border-charcoal-700">Follow-up</Badge>
      case 'not_interested':
  ...
  if (activities.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-warm-500 dark:text-cream-400">No recent activity</p>
      </div>
    )
  }
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-0.5">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-warm-900 dark:text-cream-100">{activity.prospectName}</p>
                {activity.title && (
                  <p className="text-xs text-warm-500 dark:text-cream-400 mt-0.5 truncate">{activity.title}</p>
                )}
              </div>
            ...
            {activity.description && (
              <p className="text-xs text-warm-500 dark:text-cream-400 line-clamp-2 mt-1">
                {activity.description}
              </p>
            )}
